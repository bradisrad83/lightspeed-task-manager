<x-layout>
    <div class="w-full md:w-3/4 px-4 py-2 md:py-10 md:px-8">
        <h2 class="text-gray-600 font-bold text-2xl mb-2">{{ $project->title }}</h2>
        <div class="grid grid-cols-4 md:grid-cols-5">   
            ...
            @foreach($project->tasks as $task)
                ...
                <div class="py-2 px-4 text-gray-700 md:col-span-2 border-y-2 border-l-2 border-slate-400"><a href="{{ route('users.show', ['user' => $task->user] )}}" class="cursor-pointer hover:underline hover:text-blue-800">{{ $task->user->first_name }}</a></div>
